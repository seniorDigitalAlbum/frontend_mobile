...
export const guardianApiService = {
  ...
  searchKakaoFriends: async (request: SearchKakaoFriendsRequest): Promise<SeniorInfo[]> => {
    try {
      const response = await apiClient.post<SeniorInfo[]>('/api/guardian/search-kakao-friends', request);
      console.log('🧪 카카오 친구 중 시니어 검색 API 응답:', response.data);
  ...
  connectSenior: async (request: ConnectSeniorRequest): Promise<ConnectResponse> => {
    try {
      const response = await apiClient.post<ConnectResponse>('/api/guardian/connect-senior', request);
      console.log('🧪 시니어 연결 API 응답:', response.data);
      return response.data;
    } catch (error) {
  ...
  getConnectedSeniors: async (guardianId: number): Promise<SeniorInfo[]> => {
    try {
      const response = await apiClient.get<SeniorInfo[]>(`/api/guardian/connected-seniors/${guardianId}`);
      console.log('🧪 연결된 시니어 목록 API 응답:', response.data);
      return response.data;
    } catch (error) {
